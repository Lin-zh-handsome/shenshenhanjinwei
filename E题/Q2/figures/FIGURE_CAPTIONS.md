...
1. **`fig_missing_type`：缺失模态组合。** 每条样本在有效区间中部遮挡长度约 30% 的连续区间，分别遮挡 T（文本）、A（音频）、V（视觉）、T+A、T+V、A+V。文本缺失对两种模型的影响普遍较大。数据：`outputs/q2_best_ablation/R0_selected_clean/missing_modality_analysis.csv` 和 `R3_reconstruction` 下同名文件。
