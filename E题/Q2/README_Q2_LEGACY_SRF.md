...
本文件保留旧版连续缺失实验的原始运行说明，供论文对照与复核。当前验证集分类表现最高的可部署模型是 BERT 后四层微调版本，见 `README_Q2.md`。旧版 SRF-MSA 的鲁棒性曲线、附件 3 预测与最佳 BERT 模型不是同一 checkpoint，不得合并成同一模型的结论。
...
编辑 `configs/q2_aligned.yaml` 中的 `paths.aligned_pkl`、`paths.attachment3_dir` 和 `paths.output_dir`。配置中的服务器绝对路径须改为当前机器路径。依赖版本范围记录于 Git 中的 `requirements.txt`。如确需补装 Python 包，使用国内源：
