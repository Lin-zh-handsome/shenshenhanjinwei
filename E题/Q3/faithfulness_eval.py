...
@torch.no_grad()
def run_faithfulness(cfg, checkpoint, output_dir=None, write_files=True):
    ...
    for original in tqdm(loader, desc="valid faithfulness"):
        ...
        for repeat in range(cfg["explanation"]["random_control_repeats"]):
            random_positions = {m: rng.choice(length, size=len(selected[m]), replace=False).tolist() for m in MODALITIES}
        ...
        detail_rows.append({"id": sample_id, "true_class": true_class, "pred_class": predicted_class,
                            ...
                            **{f"router_{m}": float(router[j]) for j, m in enumerate(MODALITIES)},
                            ...
                            "main_evidence_span": str(spans[MODALITIES[int(importance.argmax())]][0]),
    ...
    if write_files:
        ...
        save_json(output / "valid_faithfulness.json", summary)
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    run_faithfulness(load_config(args.config), args.checkpoint)
