...
class SpanGeometryEncoder(nn.Module):
    ...
    def forward(self, missing_mask, valid_mask):
        ...
        for i in range(b):
            ...
            missing_ratio = float((missing_mask[i] & valid_mask[i]).sum()) / max(length, 1)
            ...
            for start_tensor in starts:
                ...
                features[i, start:end, 0] = missing_ratio
                features[i, start:end, 1] = (pos - start + 1) / max(length, 1)
                features[i, start:end, 2] = (end - pos) / max(length, 1)
                features[i, start:end, 3] = span / max(length, 1)
